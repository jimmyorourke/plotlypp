...
The HTML emitted by Plotly++ requires the Plotly.js Javascript library in order to render in a web browser. Plotly++'s HTML output instructs the web browser to first try to acquire and use Plotly.js from the official CDN. If the CDN is unreachable (eg you are offline), the browser will next try to load Plotly.js from a local file in a `js` subdirectory of the directory containing the HTML file. Plotly++ emits the require file as `js/plotly.min.js` when `Figure.writeHtmlFigure()` is called with parameter `includeJsResources` set to true (the default).
