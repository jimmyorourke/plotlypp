...
class Cone {
 public:
    ...
    // vector field) or as *absolute* value (in the same units as the vector field).
    // - Default: scaled
    ...
    // same factor. With `sizemode` set to *scaled*, `sizeref` is unitless, its default value is *0.5* With `sizemode`
    // set to *absolute*, `sizeref` has the same units as the u/v/w vector field, its the default value is half the
