...
class Scatter3d {
 public:
    ...
    std::string to_string(Surfaceaxis e) {
        switch (e) {
        case Surfaceaxis::_1:
            return "-1";
    ...
    class Error_z {
     public:
        ...
        // (horizontal) bars Values are plotted relative to the underlying data.
        template <typename T, typename = std::enable_if_t<std::is_arithmetic_v<T>>>
        Error_z& arrayminus(std::vector<T> f) {
            json["arrayminus"] = std::move(f);
            return *this;
        }

        // Sets the source reference on Chart Studio Cloud for `arrayminus`.
        Error_z& arrayminussrc(std::string f) {
            json["arrayminussrc"] = std::move(f);
            return *this;
        }

        // Sets the source reference on Chart Studio Cloud for `array`.
        Error_z& arraysrc(std::string f) {
            json["arraysrc"] = std::move(f);
            return *this;
        }

        // Sets the stoke color of the error bars.
        Error_z& color(std::string f) {
            json["color"] = std::move(f);
            return *this;
        }
        ...
        // Determines whether or not the error bars have the same length in both direction (top/bottom for vertical
        // bars, left/right for horizontal bars.
        Error_z& symmetric(bool f) {
            json["symmetric"] = std::move(f);
            return *this;
        }

        // Sets the thickness (in px) of the error bars.
        Error_z& thickness(double f) {
            json["thickness"] = std::move(f);
            return *this;
        }

        Error_z& traceref(int f) {
            json["traceref"] = std::move(f);
            return *this;
        }

        Error_z& tracerefminus(int f) {
            json["tracerefminus"] = std::move(f);
            return *this;
        }
        ...
        // *constant*) corresponding to the lengths of the error bars in the bottom (left) direction for vertical
        // (horizontal) bars
        Error_z& valueminus(double f) {
            json["valueminus"] = std::move(f);
            return *this;
        }

        // Determines whether or not this set of error bars is visible.
        Error_z& visible(bool f) {
            json["visible"] = std::move(f);
            return *this;
        }

        // Sets the width (in px) of the cross-bar at both ends of the error bars.
        Error_z& width(double f) {
            json["width"] = std::move(f);
            return *this;
        }
    ...
    class Marker {
     public:
        ...
        // Sets the marker opacity. Note that the marker opacity for scatter3d traces must be a scalar value for
        // performance reasons. To set a blending opacity value (i.e. which is not transparent), set *marker.color* to
    ...
    class Projection {
     public:
        ...
        Projection& x(class X f) {
            json["x"] = std::move(f.json);
            return *this;
        }

        Projection& y(class Y f) {
            json["y"] = std::move(f.json);
            return *this;
        }

        Projection& z(class Z f) {
    ...
    Scatter3d& transforms(class Transforms f) {
