 ...
 public:
    ...
    class Line {
     public:
        ...
        // *longdashdot*) or a dash length list in px (eg *5px,10px,2px,2px*). Note that this style setting can also be
        // set per direction via `increasing.line.dash` and `decreasing.line.dash`.
        Line& dash(std::string f) {
            json["dash"] = std::move(f);
            return *this;
        }
    ...
    Ohlc& transforms(class Transforms f) {
